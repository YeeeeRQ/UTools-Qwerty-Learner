...
import type { Dictionary } from '@/typings/index'
...
export let localDictionaries: Dictionary[] = []
...
export let dictionaries: Dictionary[] = [...localDictionaries, ...builtinDictionaries]
...
export const pushDictionaries = (dictResource: DictionaryResource) => {
  const dict: Dictionary = {
    ...dictResource,
    chapterCount: calcChapterCount(dictResource.length),
  }
  localDictionaries.push(dict)
  Object.assign(idDictionaryMap, Object.fromEntries([[dict.id, dict]]))
}

export const initLocalDictionries = () => {
  if (!window.readLocalDictConfig) return

  const config = window.readLocalDictConfig()

  localDictionaries = []
  for (const dictResource of config) {
    pushDictionaries(dictResource as DictionaryResource)
  }
  dictionaries = [...localDictionaries, ...builtinDictionaries]
}
initLocalDictionries()

window.initLocalDictionries = initLocalDictionries
